...
export async function POST(request: Request) {
  try {
    ...
    // Read key inside handler — never at module level
    ...
    if (!usage || usage.searches_remaining <= 0) {
      return new Response(
        JSON.stringify({ error: "no_searches", message: "No searches remaining. Please upgrade." }),
        { status: 402, headers: { "Content-Type": "application/json" } }
      );
    }

    const systemPrompt = `You are VibeFlow's AI marketing director — an expert at creating complete marketing campaigns for indie hackers and vibe coders.
          ...
          const res = await fetch("https://api.anthropic.com/v1/messages", {
            ...
            body: JSON.stringify({
              ...
              messages: [
                {
                  role: "user",
                  content: `Generate a complete marketing campaign for this app:\n\n${prompt}`,
                },
              ],
            }),
          });
              ...
              try {
                ...
                if (parsed.type === "message_stop") {
                  ...
                  const { data: campaign } = await supabase
                    .from("campaigns")
                    .insert({ user_id: user.id, prompt, content: fullContent })
                    .select()
                    .single();
                  ...
                  controller.enqueue(
                    encoder.encode(
                      `data: ${JSON.stringify({ done: true, campaignId: campaign?.id })}\n\n`
                    )
                  );
                  controller.close();
                  return;
