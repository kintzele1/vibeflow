...
const AFFILIATE_TYPES = [
  { id: "program_setup",        label: "Program Setup",          icon: "🤝", desc: "Commission, payout, T&Cs, tracking platform pick" },
  { id: "recruitment",          label: "Recruitment Campaign",   icon: "🎯", desc: "Prospect list + 4 outreach templates + fit scoring" },
  { id: "asset_kit",            label: "Affiliate Asset Kit",    icon: "🎁", desc: "Swipe copy, social templates, review + video scripts" },
  { id: "performance_report",   label: "Performance Framework",  icon: "📈", desc: "Metrics, tiers, payout simulator, optimization plays" },
];
...
export default function AffiliatePage() {
  ...
  useEffect(() => {
    ...
    if (p) setPrompt(p);
    if (t && AFFILIATE_TYPES.some(s => s.id === t)) setSelected(t);
  ...
  async function handleGenerate() {
    ...
    try {
      const response = await fetch("/api/affiliate", {
        ...
        body: JSON.stringify({ prompt, affiliateType: selected, applyBrandKit }),
      });
  ...
  function handleExport() {
    const label = AFFILIATE_TYPES.find(t => t.id === selected)?.label ?? "affiliate";
  ...
  const selectedType = AFFILIATE_TYPES.find(t => t.id === selected);
    ...
    <div style={{ padding: "40px 48px", maxWidth: 900, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <p style={{ fontFamily: "var(--font-dm-sans)", fontSize: 16, color: "#878787" }}>
          Launch and scale an affiliate / referral program — program setup, partner recruitment, asset kit, performance framework.
        </p>
      </div>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <span style={{ fontFamily: "var(--font-dm-sans)", fontSize: 13, color: "#AAAAAA" }}>
                {selected ? `Generating: ${selectedType?.label}` : "Select an affiliate output above"} · 1 search
              </span>
              <button
              ...
              >
                {loading ? "Generating..." : `Generate ${selectedType?.label ?? "Affiliate"} →`}
              </button>
            </div>
          </div>
        </>
      ...
      {loading && !output && (
        <div style={{
          ...
          <p style={{ fontFamily: "var(--font-dm-sans)", fontSize: 14, color: "#878787" }}>
            Real program mechanics, outreach-ready, brand-aligned.
          </p>
        </div>
      )}
