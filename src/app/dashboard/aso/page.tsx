...
export default function AsoPage() {
  ...
  useEffect(() => {
    ...
    if (p) setPrompt(p);
    if (t && ASO_TYPES.some(s => s.id === t)) setSelected(t);
  ...
  async function handleGenerate() {
    ...
    try {
      const response = await fetch("/api/aso", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ prompt, asoType: selected, applyBrandKit }),
      });
  ...
  function handleExport() {
    const label = ASO_TYPES.find(t => t.id === selected)?.label ?? "aso";
  ...
  const selectedType = ASO_TYPES.find(t => t.id === selected);
    ...
    <div style={{ padding: "40px 48px", maxWidth: 900, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <p style={{ fontFamily: "var(--font-dm-sans)", fontSize: 16, color: "#878787" }}>
          App Store + Google Play optimization — titles, keywords, descriptions, screenshots, videos.
        </p>
      </div>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <span style={{ fontFamily: "var(--font-dm-sans)", fontSize: 13, color: "#AAAAAA" }}>
                {selected ? `Generating: ${selectedType?.label}` : "Select an ASO output above"} · 1 search
              </span>
              <button
              ...
              >
                {loading ? "Generating..." : `Generate ${selectedType?.label ?? "ASO"} →`}
              </button>
            </div>
          </div>
        </>
      ...
      {loading && !output && (
        <div style={{
          ...
          <p style={{ fontFamily: "var(--font-syne)", fontWeight: 700, fontSize: 18, color: "#1F1F1F", marginBottom: 8 }}>
            Optimizing your {selectedType?.label.toLowerCase()}...
          </p>
          <p style={{ fontFamily: "var(--font-dm-sans)", fontSize: 14, color: "#878787" }}>
            App Store + Google Play ready. Ship-grade copy.
          </p>
        </div>
      )}
