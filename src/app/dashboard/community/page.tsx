...
const COMMUNITY_TYPES = [
  { id: "product_hunt",       label: "Product Hunt Kit",     icon: "🚀", desc: "Tagline, description, maker story, hunter outreach, 24h plan" },
  { id: "influencer_outreach",label: "Influencer Outreach",  icon: "🎤", desc: "10 targeted creators + 3 DM templates + follow-up + attribution" },
  { id: "reddit_discord",     label: "Reddit + Discord",     icon: "👾", desc: "6 subreddits + 4 Discord servers + 5 ready posts + rules" },
  { id: "pr_pitch",           label: "PR Pitch Package",     icon: "📰", desc: "5 journalists + 2 pitch emails + press kit checklist" },
  { id: "launch_x_thread",    label: "Launch X Thread",      icon: "𝕏",  desc: "15-post launch thread + timing + engagement plan" },
];
...
export default function CommunityPage() {
  ...
  useEffect(() => {
    ...
    if (p) setPrompt(p);
    if (t && COMMUNITY_TYPES.some(s => s.id === t)) setSelected(t);
  ...
  async function handleGenerate() {
    ...
    try {
      const response = await fetch("/api/community", {
        ...
        body: JSON.stringify({ prompt, communityType: selected, applyBrandKit }),
      });
  ...
  function handleExport() {
    const label = COMMUNITY_TYPES.find(t => t.id === selected)?.label ?? "community";
  ...
  const selectedType = COMMUNITY_TYPES.find(t => t.id === selected);
    ...
    <div style={{ padding: "40px 48px", maxWidth: 900, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <p style={{ fontFamily: "var(--font-dm-sans)", fontSize: 16, color: "#878787" }}>
          Product Hunt kits, influencer outreach, Reddit + Discord strategy, PR, launch threads — ready to ship.
        </p>
      </div>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <span style={{ fontFamily: "var(--font-dm-sans)", fontSize: 13, color: "#AAAAAA" }}>
                {selected ? `Generating: ${selectedType?.label}` : "Select a launch asset above"} · 1 search
              </span>
              <button
              ...
              >
                {loading ? "Generating..." : `Generate ${selectedType?.label ?? "Launch Kit"} →`}
              </button>
            </div>
          </div>
        </>
      ...
      {loading && !output && (
        <div style={{
          ...
          <p style={{ fontFamily: "var(--font-syne)", fontWeight: 700, fontSize: 18, color: "#1F1F1F", marginBottom: 8 }}>
            Building your {selectedType?.label.toLowerCase()}...
          </p>
          <p style={{ fontFamily: "var(--font-dm-sans)", fontSize: 14, color: "#878787" }}>
            Real accounts, real copy, launch-day ready.
          </p>
        </div>
      )}
