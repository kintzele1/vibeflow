...
const EMAIL_TYPES = [
  { id: "welcome",      label: "Welcome Sequence",       icon: "👋", desc: "3-email welcome series after signup" },
  { id: "onboarding",   label: "Onboarding Sequence",    icon: "🚀", desc: "7-day progressive getting-started flow" },
  { id: "upsell",       label: "Upsell Sequence",        icon: "💰", desc: "5-email free → paid conversion" },
  { id: "reengagement", label: "Re-engagement",          icon: "💌", desc: "3-email win-back for lapsed users" },
  { id: "broadcast",    label: "Broadcast Email",        icon: "📢", desc: "One-off launch, update, or announcement" },
];
...
export default function EmailPage() {
  ...
  useEffect(() => {
    ...
    if (p) setPrompt(p);
    if (t && EMAIL_TYPES.some(s => s.id === t)) setSelected(t);
  ...
  async function handleGenerate() {
    ...
    try {
      const response = await fetch("/api/email", {
        ...
        body: JSON.stringify({ prompt, emailType: selected, applyBrandKit }),
      });
  ...
  function handleExport() {
    const label = EMAIL_TYPES.find(t => t.id === selected)?.label ?? "email";
  ...
  const selectedType = EMAIL_TYPES.find(t => t.id === selected);
    ...
    <div style={{ padding: "40px 48px", maxWidth: 900, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <p style={{ fontFamily: "var(--font-dm-sans)", fontSize: 16, color: "#878787" }}>
          Welcome flows, onboarding, upsells, win-back, broadcasts — in your voice.
        </p>
      </div>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <span style={{ fontFamily: "var(--font-dm-sans)", fontSize: 13, color: "#AAAAAA" }}>
                {selected ? `Generating: ${selectedType?.label}` : "Select an email output above"} · 1 search
              </span>
              <button
              ...
              >
                {loading ? "Generating..." : `Generate ${selectedType?.label ?? "Email"} →`}
              </button>
            </div>
          </div>
        </>
      ...
      {loading && !output && (
        <div style={{
          ...
          <p style={{ fontFamily: "var(--font-syne)", fontWeight: 700, fontSize: 18, color: "#1F1F1F", marginBottom: 8 }}>
            Writing your {selectedType?.label.toLowerCase()}...
          </p>
          <p style={{ fontFamily: "var(--font-dm-sans)", fontSize: 14, color: "#878787" }}>
            Subject lines, body copy, CTAs — ready to paste into your ESP.
          </p>
        </div>
      )}
