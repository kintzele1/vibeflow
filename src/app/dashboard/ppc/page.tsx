...
const PPC_TYPES = [
  { id: "google",   label: "Google Ads",    icon: "🎯", desc: "Headlines, keywords, extensions, budget split" },
  { id: "meta",     label: "Meta Ads",      icon: "📘", desc: "FB + IG copy, creative concepts, audiences" },
  { id: "linkedin", label: "LinkedIn Ads",  icon: "in", desc: "B2B targeting, intro text, lead gen" },
  { id: "x",        label: "X Ads",         icon: "𝕏",  desc: "Promoted posts, A/B test matrix" },
  { id: "tiktok",   label: "TikTok Ads",    icon: "🎵", desc: "Video scripts, creator strategy, hooks" },
];
...
export default function PpcPage() {
  ...
  useEffect(() => {
    ...
    if (p) setPrompt(p);
    if (t && PPC_TYPES.some(s => s.id === t)) setSelected(t);
  ...
  async function handleGenerate() {
    ...
    try {
      const response = await fetch("/api/ppc", {
        ...
        body: JSON.stringify({ prompt, ppcPlatform: selected, applyBrandKit }),
      });
  ...
  function handleExport() {
    const label = PPC_TYPES.find(t => t.id === selected)?.label ?? "ppc";
  ...
  const selectedType = PPC_TYPES.find(t => t.id === selected);
    ...
    <div style={{ padding: "40px 48px", maxWidth: 900, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <p style={{ fontFamily: "var(--font-dm-sans)", fontSize: 16, color: "#878787" }}>
          Full ad campaigns — copy, targeting, budget, A/B tests — tailored to each platform's playbook.
        </p>
      </div>
        ...
        <>
          ...
          <div style={{
            ...
            <div style={{
              ...
              >
                {loading ? "Generating..." : `Generate ${selectedType?.label ?? "Ads"} →`}
              </button>
            </div>
          </div>
        </>
      ...
      {loading && !output && (
        <div style={{
          ...
          <p style={{ fontFamily: "var(--font-syne)", fontWeight: 700, fontSize: 18, color: "#1F1F1F", marginBottom: 8 }}>
            Building your {selectedType?.label} campaign...
          </p>
          <p style={{ fontFamily: "var(--font-dm-sans)", fontSize: 14, color: "#878787" }}>
            Copy, targeting, budget, and A/B ideas — all platform-native.
          </p>
        </div>
      )}
